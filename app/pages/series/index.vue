<template>
  <div class="space-y-12">
    <header class="space-y-4">
      <h1 class="text-5xl font-black">Séries</h1>
    ...
    <div v-else class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
      <MediaCard v-for="serie in sortedSeries" :key="serie.id" :item="serie" type="series" />
    </div>
  </div>
</template>
...
const sortedSeries = computed(() => data.value?.byGenre || [])
</script>
